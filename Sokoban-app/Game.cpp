...
bool Game::InputBoardData()
{
	...
	// テキストボックスとスライダーで調整できるようにするパラメータを表す構造体
	struct BoardInfo
	{
		std::string mName;		// パラメータ名
		double mMin;			// 値の最小値
		double mMax;			// 値の最大値
		double mInitialValue;	// 初期値
		sf::Vector2i mPos;		// ウィジェットの位置
		bool mIsInteger;		// 整数値のパラメータか
	};
...
{
	// Windows環境でDPIスケールを取得する例
	float dpiScale = 1.0f;
	HWND hwnd = GetActiveWindow();
	if (hwnd)
	{
		HDC hdc = GetDC(hwnd);
		int dpi = GetDeviceCaps(hdc, LOGPIXELSX); // 水平方向のDPIを取得
		dpiScale = dpi / 96.0f; // 標準DPI(96)に基づいてスケールを計算
		ReleaseDC(hwnd, hdc);
	}
	return dpiScale;
}
